...
package net.mcreator.levraimod.keybind;
...
import net.mcreator.levraimod.procedures.BossToucheEnfonceeProcedure;
...
public class BossKeyBinding extends LeVraiModModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public BossKeyBinding(LeVraiModModElements instance) {
		super(instance, 183);
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					LeVraiModMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
	...
	private static void pressAction(PlayerEntity entity, int type, int pressedms) {
		...
		if (type == 0) {
			...
			BossToucheEnfonceeProcedure.executeProcedure(Stream
